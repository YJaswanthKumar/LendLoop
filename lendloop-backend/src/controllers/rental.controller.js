...
async function cancelRental(req, res, next) {
  try {
    const rental = await rentalService.cancelRental(req.params.id, req.user.id, req.body);
...
async function resolveDeposit(req, res, next) {
  try {
    const rental = await rentalService.resolveDeposit(req.params.id, req.user.id, req.body);
    return success(res, 200, 'Deposit resolved successfully', { rental });
  } catch (err) {
    return next(err);
  }
}
