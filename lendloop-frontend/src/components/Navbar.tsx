    ...
    <header className="sticky top-0 z-40 border-b border-border bg-background/90 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-2 px-4">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          <Link to="/browse" className={navLink} activeProps={{ className: `${navLink} text-foreground` }}>
            Browse
          </Link>
          <Link to="/map" className={navLink} activeProps={{ className: `${navLink} text-foreground` }}>
            Map
          </Link>
          {isAuthenticated && (
            <>
      ...
      {open && (
        <nav className="border-t border-border bg-background px-4 py-3 md:hidden">
          <div className="flex flex-col gap-1">
            <Link to="/browse" onClick={() => setOpen(false)} className="rounded-lg px-3 py-2.5 text-sm font-medium hover:bg-muted">
              Browse items
            </Link>
            <Link to="/map" onClick={() => setOpen(false)} className="rounded-lg px-3 py-2.5 text-sm font-medium hover:bg-muted">
              Map view
            </Link>
            {isAuthenticated ? (
              <>
