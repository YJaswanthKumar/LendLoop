...
export async function cancelRental(id: string, reason?: string) {
  const res = await api.patch<ApiEnvelope<{ rental: Rental }>>(
    ENDPOINTS.rentals.cancel(id),
...
export async function resolveDeposit(
  ...
  const res = await api.patch<ApiEnvelope<{ rental: Rental }>>(ENDPOINTS.rentals.deposit(id), {
    status,
    refundAmount,
